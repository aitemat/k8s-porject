...
- 103.101.162.200 k8s-master2: làm Slave master
- 45.117.80.150 k8s-master3: làm chức năng HA
- 103.101.163.72 k8s-nodew1: share NFS
...
`sudo kubeadm init --control-plane-endpoint="<IP-master-primary>:6443" --upload-certs --pod-network-cidr=<pod-network-cidr>`  
Trong đó:
- `<IP-master-primary>` là địa chỉ IP của load balancer nếu bạn sử dụng load balancer, hoặc là địa chỉ IP của node master đầu tiên nếu bạn không sử dụng load balancer. 
...
```sh
kubeadm join 45.117.80.52:6443 --token rf2e2b.bmih5kdy1cvrhz2a \
        --discovery-token-ca-cert-hash sha256:af385be97ee6d93b4d8e21fe8d98174a50cdf4a1c18510b3a8a981d53ee46f1d
```
Lệnh dùng cho node master join vào:  
```sh
 kubeadm join 45.117.80.52:6443 --token rf2e2b.bmih5kdy1cvrhz2a \
        --discovery-token-ca-cert-hash sha256:af385be97ee6d93b4d8e21fe8d98174a50cdf4a1c18510b3a8a981d53ee46f1d \
        --control-plane --certificate-key 3d30b618c34469607b67babad36b8ecafc113e354868864c2f43fa2856bf30c2
```
...
`sudo kubeadm join <IP-master-primary>:6443 --token <token-value> --discovery-token-ca-cert-hash sha256:<hash-value> --control-plane --certificate-key <certificate-key-value>`  
Trong đó:  
`<IP-master-primary>` là địa chỉ IP của load balancer nếu bạn sử dụng load balancer, hoặc là địa chỉ IP của node master đầu tiên nếu bạn không sử dụng load balancer.  
