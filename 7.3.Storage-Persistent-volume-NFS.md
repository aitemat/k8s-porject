...
- `name: nfs-pv`: đặt tên cho Persistent Volume này là "nfs-pv".  
...
nfs-pv   5Gi        RWX            Retain           Bound    default/pvc-nginx                           4m3s
...
pvc-nginx   Bound    nfs-pv   5Gi        RWX                           18m
...
spec:
  ...
  template:
    ...
    spec:
      ...
      nodeSelector:
        kubernetes.io/hostname: k8s-nodew1 # chọn cố định node worker nfs sẽ deploy pod replicas=2, khi đó 2 pod này sẽ tạo trên cùng 1 node worker
      volumes:
        - name: volume-nginx
...
Có thể sử dụng lệnh: `kubectl describe pod <tên pod>`  đễ phân tích bên trong.
ví dụ:  `kubectl describe pod nginx-app-7c596d479d-l5bls`  

```sh
root@k8s-master1:~# kubectl get pods
NAME                         READY   STATUS    RESTARTS   AGE
k8s-nginx-76d747d8d8-jw54v   1/1     Running   0          6h59m
nginx-app-7c596d479d-7xmjh   1/1     Running   0          6s
`nginx-app-7c596d479d-l5bls`   1/1     Running   0          6s
```
...
<img src="/images/nginx-deployment6.jpg">  
